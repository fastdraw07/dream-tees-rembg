...
# Load the u2net model ONCE at startup and reuse it for every request. Creating
# the session per-request is what makes naive RemBG servers slow. This is the
# same default model ("u2net") used in the manual testing.
...
def health():
    ...
    return {"status": "ok", "service": "dream-tees-rembg", "model": "u2net"}
...
def remove_background(req: RemoveRequest):
    ...
    try:
        # Open, ensure RGBA, run removal, return PNG (PNG preserves transparency).
